...
if (!class_exists('Happfoco_Api')) {
	...
	class Happfoco_Api
	{
		...
		const API_ENDPOINT_DEMO_VALIDATE = 'https://webhook.site/6541565f-7691-4e97-98f8-64b96998d410';

		// Demo endpoint for receiving form submissions in testing mode
		// Replace the token below with your own from https://webhook.site
		const API_ENDPOINT_DEMO_CREATE_CONTACT = 'https://webhook.site/6541565f-7691-4e97-98f8-64b96998d410';
			...
			if (!empty($existing_api_key)) {
				...
				try {
					...
					if (!empty($decrypted)) {
						...
						update_option('happfoco_encryption_key_hash', base64_encode($new_key), false);
		...
		/**
		 * Check if the plugin is running in demo/test mode.
		...
		public function is_demo_mode($api_key = null)
		{
			...
			return ($api_key === self::DEMO_API_KEY);
		}
		...
		public function register_routes()
		{
			...
			register_rest_route('happfoco/v1', '/save-api-config', array(
				'methods' => 'POST',
				'callback' => array($this, 'happfoco_save_api_settings'),
				'permission_callback' => array($this, 'check_permission'),
			));

			register_rest_route('happfoco/v1', '/save-form-settings', array(
				'methods' => 'POST',
				'callback' => array($this, 'happfoco_save_form_settings'),
				'permission_callback' => array($this, 'check_permission'),
			));

			register_rest_route('happfoco/v1', '/fetch-form-fields', array(
				'methods' => 'POST',
				'callback' => array($this, 'happfoco_get_form_fields'),
				'permission_callback' => array($this, 'check_permission'),
			));

			register_rest_route('happfoco/v1', '/get-api-config', array(
				'methods' => 'GET',
				'callback' => array($this, 'happfoco_get_api_settings'),
				'permission_callback' => array($this, 'check_permission'),
			));

			register_rest_route('happfoco/v1', '/fetch-forms', array(
				'methods' => 'GET',
				'callback' => array($this, 'happfoco_get_forms'),
				'permission_callback' => array($this, 'check_permission'),
			));

			register_rest_route('happfoco/v1', '/fetch-form-data', array(
				'methods' => 'GET',

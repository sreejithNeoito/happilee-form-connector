...
if (!class_exists('Happfoco_Api')) {
	...
	class Happfoco_Api
	{
		...
		public const API_ENDPOINT_CREATE_CONTACT = 'https://api.happilee.io/api/v1/createContact';

		// Endpoint used to get templates
		const API_ENDPOINT_GET_TEMPLATES = 'https://api.happilee.io/api/v1/getTemplateMessages';

		// Endpoint used to save templates
		const API_ENDPOINT_SAVE_TEMPLATES = 'https://api.happilee.io/api/v1/sendMessage';
		...
		public function register_routes()
		{
			...
			/**
			 * Handle saving form settings via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				'/save-form-settings',
			...
			/**
			 * Handle saving template settings via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				...
				array(
					'methods' => 'POST',
					'callback' => array($this, 'happfoco_save_template_settings'),
					'permission_callback' => array($this, 'check_permission'),
					'args' => array(
						'form_id' => array(
							'required' => true,
			...
			/**
			 * Handle fetching form fields via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				'/fetch-form-fields',
			...
			/**
			 * Handle fetching API configuration via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				'/get-api-config',
			...
			/**
			 * Handle fetching forms via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				'/fetch-forms',
			...
			/**
			 * Handle fetching form data via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				'/fetch-form-data',
			...
			/**
			 * Handle fetching template messages via REST API
			 */
			register_rest_route(
				'happfoco/v1',
			...
			/**
			 * Handle fetching template settings via REST API
			 */
			register_rest_route(
				'happfoco/v1',
				...
				array(
					'methods' => 'GET',
					'callback' => array($this, 'happfoco_fetch_template_settings'),
					'permission_callback' => array($this, 'check_permission'),
				)
			);
		 ...
		 * @param WP_REST_Request $request Full request object.
		 * @return WP_REST_Response Template settings saved or an error response.
		 */
		public function happfoco_save_template_settings(WP_REST_Request $request)
		{
			global $wpdb;
			...
			$param_mappings = $request->get_param('param_mappings');

			if (empty($form_id) || empty($form_type) || empty($template_id)) {
				return new WP_REST_Response(
					array(
						'success' => false,
						'message' => __('form_id, form_type and template_id are required', 'happilee-forms-connector'),
					),
					400
				);
			}

			// Sanitize each param → field_value pair
			$sanitized_mappings = array();
			if (is_array($param_mappings) && !empty($param_mappings)) {
				foreach ($param_mappings as $param => $field_value) {
					$sanitized_mappings[sanitize_text_field($param)] = sanitize_text_field($field_value);
				}
			}
			...
			if ($existing) {
				...
				$result = $wpdb->update(
					...
					array(
						...
						'param_mappings' => $param_mappings_json,
				...
				if (false === $result) {
					return new WP_REST_Response(
						array(
							'success' => false,
							'message' => __('Failed to update template settings', 'happilee-forms-connector'),
						),
						500
					);
				}
				...
				return new WP_REST_Response(
					array(
						'success' => true,
						'message' => __('Template settings updated successfully', 'happilee-forms-connector'),
						'action' => 'updated',
					),
					200
				);
				...
				if (false === $result) {
					return new WP_REST_Response(
						array(
							'success' => false,
							'message' => __('Failed to save template settings', 'happilee-forms-connector'),
						),
						500
					);
				}
				...
				return new WP_REST_Response(
					array(
						'success' => true,
						'message' => __('Template settings saved successfully', 'happilee-forms-connector'),
						'action' => 'inserted',
					),
					200
				);
		 ...
		 * @param WP_REST_Request $request Full request object.
		 * @return WP_REST_Response Template settings or an error response.
		 */
		public function happfoco_fetch_template_settings(WP_REST_Request $request)
		{
			global $wpdb;
			...
			if ($wpdb->last_error) {
				return new WP_REST_Response(
					array(
						'success' => false,
						'message' => __('Failed to fetch template settings', 'happilee-forms-connector'),
					),
					500
				);
			}
			...
			return new WP_REST_Response(
				array(
					...
					'message' => __('Template settings fetched successfully', 'happilee-forms-connector'),
				),
				200
			);
		}

...
if (!class_exists('Happilee_HFC_Api')) {
	...
	class Happilee_HFC_Api
	{
		...
		const API_ENDPOINT = 'https://webhook.site/0e4c33ec-11fa-4cae-aa9d-9ffd438ed555';
			...
			if (!empty($existing_api_key)) {
				...
				try {
					...
					if (!empty($decrypted)) {
						...
						error_log('Happilee Forms Connect: Successfully migrated to new encryption key');

						return $new_key;
					}
				} catch (Exception $e) {
					error_log('Happilee Forms Connect: Migration failed - ' . $e->getMessage());
				}
			}
